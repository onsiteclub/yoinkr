...
- ~~Deal só nasce de application em job~~ **RESOLVIDO 2026-07-23 (ofertas diretas + parceria):** tabela `yoinkr.offers` — o hirer oferece trabalho direto do perfil do worker ("Offer work", modal com pitch + rate), a oferta cai no chat, o worker aceita/recusa numa barra e o aceite cria o deal (`application_id` null; deal sem listing ganha barra "Direct deal" no chat para done/rate). Uma oferta pendente por par (índice parcial). Bots-worker aceitam ofertas em persona (bot-reply). E jobs ganharam o toggle **Partnership** ("não é ajudante, é sócio temporário"): badge `PARTNER` no card, ação "Team up", mesmo ciclo de deal/avaliação.
